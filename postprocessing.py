...
import numpy as np
data_LSTM = pd.read_csv("./data/test_data/tts_1_Informer_v1.0.csv")
data_conformer = pd.read_csv("./data/test_data/tts_1_Informer_v1.0.csv")
...
#plotExponentialSmoothing(data_conformer['JawOpen'], data_LSTM['JawOpen'], alphas=[0.4],)   
#plotDoubleExponentialSmoothing( data_conformer['JawOpen'][:200], data_LSTM['JawOpen'][:200], alphas=[0.5,0.3], betas=[0.9,0.3])
print(data_conformer.head())
data_conformer[MOUTH_BS] = data_conformer[MOUTH_BS].apply(exponential_smoothing, alpha=0.5,axis=0)
